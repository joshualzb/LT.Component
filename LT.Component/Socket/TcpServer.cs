...
namespace LT.Component.Socket
{
    ...
    public class TcpServer
    {
        ...
        public TcpServer(int listenPort, SslConfig sslConfig)
        {
        ...
        public TcpServer(string listenIP, int listenPort, SslConfig sslConfig)
        {
        ...
        public void Start()
        {
            ...
            ThreadPool.QueueUserWorkItem(delegate(object state)
            {
                while (true)
                {
                    ...
                    Thread t = new Thread(new ParameterizedThreadStart(SocketAsyncCallback));
                    t.Start(client);
        ...
        private void SocketAsyncCallback(object parameter)
        {
            ...
            // the console.
            System.Net.Sockets.TcpClient client = parameter as System.Net.Sockets.TcpClient;
            ...
            else
            {
                ...
                {
                    //执行Web请求
                    if (CommandHandler.Instance().Actions.ContainsKey("WEB"))
                    {
                        try
                        {
                            CommandHandler.Instance().Actions["WEB"].Excute(stream, 0, ip);
                        }
                        catch (Exception ex)
                        {
                            SocketHelper.SendString(stream, ex.Message);
